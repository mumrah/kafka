...
object BrokerMetadataPublisher {
  ...
  /**
   * Find the number of partitions that are in a given topic. If the topic doesn't exist,
   * return the default.
   *
   * @param metadataCache   The metadata cache to search.
   * @param topicName       The topic name.
   * @param defaultValue    The default value to use if the topic is not present.
   *
   * @return                The number of partitions.
   */
  def getNumPartitionsWithDefault(metadataCache: KRaftMetadataCache,
                                  topicName: String,
                                  defaultValue: Int): Int = {
    val numPartitions = metadataCache.numPartitions(topicName)
    if (numPartitions == 0) {
      defaultValue
    } else {
      numPartitions
...
class BrokerMetadataPublisher(conf: KafkaConfig,
  ...
  private def initializeManagers(): Unit = {
    ...
    groupCoordinator.startup(() => getNumPartitionsWithDefault(metadataCache,
      Topic.GROUP_METADATA_TOPIC_NAME, conf.offsetsTopicPartitions))

    // Start the transaction coordinator.
    txnCoordinator.startup(() => getNumPartitionsWithDefault(metadataCache,
      Topic.TRANSACTION_STATE_TOPIC_NAME, conf.transactionTopicPartitions))
  }
