...
class KRaftMetadataCache(val brokerId: Int) extends MetadataCache with Logging with ConfigRepository {
  ...
  override def getAliveBrokerNode(brokerId: Int, listenerName: String): Option[Node] = {
    Option(_currentImage.cluster().broker(brokerId)).flatMap(_.node(listenerName).asScala)
  }

  override def getAliveBrokerNodes(listenerName: String): Seq[Node] = {
    _currentImage.cluster().brokers().values().asScala.filter(!_.fenced()).flatMap(_.node(listenerName).asScala).toSeq
  }
  ...
  override def numPartitions(topicName: String): Int = {
    Option(_currentImage.topics().getTopic(topicName)).
      flatMap(topic => Some(topic.partitions().size())).
      getOrElse(0)
  }
